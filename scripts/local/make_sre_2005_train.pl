...
# Apache 2.0.
# Usage: make_sre_2006_train.pl <path to LDC2011S01> <Path to root level output dir>

if (@ARGV != 2) {
  print STDERR "Usage: $0 <path-to-LDC2011S01> <path-to-output>\n";
  print STDERR "e.g. $0 /export/corpora5/LDC/LDC2011S01 data\n";
  exit(1);
}
...
foreach (@gender_list) {
  ...
  foreach $case (@case_list) {
    $out_dir = "$out_base_dir/sre05_train_${case}_${gender}";
    mkdir "$out_dir";
    $casefile = $db_base."/".$gender."/".$case.".trn";
