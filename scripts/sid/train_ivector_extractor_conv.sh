...
#             2015  Srikanth Madikeri (Idiap Research Instittue)
# Apache 2.0.
...
if [ $stage -le -2 ]; then
  ...
  gmm-global-to-fgmm $dir/final.dubm $dir/final.dubm.f  || exit 1;
  $cmd $dir/log/init.log \
    ivector-extractor-conv-init --ivector-dim=$ivector_dim  \
     $dir/final.dubm.f $dir/0.ie || exit 1
fi 
...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj_full $dir/log/gselect.JOB.log \
    ...
    fgmm-global-gselect-to-post --min-post=$min_post $dir/final.ubm "$feats" \
       ark:- "ark:|gzip -c >$dir/post.JOB.gz" || exit 1;
else
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    echo "Accumulating stats (pass $x)"
        $cmd JOB=1:$nj_full $dir/log/acc.$x.JOB.log \
        ivector-extractor-conv-acc-stats --num-threads=$num_threads $dir/$x.ie "$feats" ark:"gunzip -c $dir/post.JOB.gz |" $dir/acc.$x.JOB.ark || exit 1
    num_complete=0
    stats_list=
    for g in $(seq $nj_full); do
        if [ -f $dir/acc.$x.$g.ark ] ; then
            stats_list="$stats_list $dir/acc.$x.$g.ark"
            ((num_complete++))
        fi
    done
	echo "Summing accs (pass $x)"
      $cmd  $dir/log/acc.$x.log \
        ivector-extractor-conv-sum-acc $stats_list \
          $dir/acc.$x || exit 1 
	...
	$cmd $dir/log/update.$x.log \
	  ivector-extractor-conv-est --num-threads=$nt $dir/$x.ie $dir/acc.$x $dir/$[$x+1].ie || exit 1;
	rm $dir/acc.$x.*
    if $cleanup; then
      rm $dir/acc.$x
      rm $dir/$x.ie
